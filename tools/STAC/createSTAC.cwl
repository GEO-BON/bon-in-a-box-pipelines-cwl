...
label: STAC Collection to Catalog
doc:
  - |
    Description:
    Converts STAC collection(s) into a STAC catalog. A STAC catalog is an aggregation of STAC collections.
  - "Lifecycle tag: Core."
  - |
    Authors:
  ...
  - |
    ...
    ${
      return JSON.stringify({
        stac_collections: (inputs.stac_collections || []).map(function(file) { return file.path; }),
    ...
    source $SCRIPT_STUBS_LOCATION/system/condaEnvironment.sh $OUTPUT_LOCATION "STAC__createSTAC" \
    "channels: [conda-forge]
    dependencies: [pystac, jsonschema]
    name: STAC__createSTAC
    ...
    source $SCRIPT_STUBS_LOCATION/system/condaPackEnvironment.sh STAC__createSTAC /conda-envs >> "$log" 2>&1
...
inputs:
  ...
  stac_name:
    type: string?
    label: STAC Name
    doc: Name of the STAC catalog to be created, using lowercase letters and hyphens only. Default is "biab-stac".
    default: biab-stac
...
outputs:
  stac_catalog_out:
    ...
    label: STAC Catalog
    doc: >
      STAC Catalog linking to the provided collections.
    ...
    outputBinding:
      ...
      outputEval: |
        ${
          var value = extractOutput(self, "stac_catalog");

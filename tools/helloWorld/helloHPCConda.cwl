...
label: HPC Python & Conda Example
doc:
  - "Description:
    Python script that runs on a HPC, with a conda sub-environment. The conda sub-environment will be resolved on the automation node, and activated on the compute node.
...
arguments:
  - |
    ...
    source $SCRIPT_STUBS_LOCATION/system/condaEnvironment.sh $OUTPUT_LOCATION "helloWorld__helloHPCConda" \
    "channels: [conda-forge]
    dependencies: [libcxx]
    name: helloWorld__helloHPCConda
    ...
    source $SCRIPT_STUBS_LOCATION/system/condaPackEnvironment.sh helloWorld__helloHPCConda /conda-envs >> "$log" 2>&1
...
inputs:
  ...
  some_csv_file:
    type: File?
    label: Some file
    doc: A file, which will be sent to HPC and printed from the compute node.
    default: /scripts/helloWorld/example.csv
  ...
  scriptPath:
    ...
    default: helloWorld/helloHPC.py
...
outputs:
  target:
    ...
    doc: >
      ...
      It is printed at the beginning of the script, this allows to test fetching partial results when the job times out.
    outputBinding:
      glob: "output.json"
      loadContents: true
      outputEval: |
        ${
          var value = extractOutput(self, "target");
  ...
  length:
    ...
    outputBinding:
      ...
      outputEval: |
        ${
          var value = extractOutput(self, "length");
          if (value === null) return null;
          return parseInt(value);
        }

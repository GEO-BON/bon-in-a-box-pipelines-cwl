...
steps:
    ...
    run:
      ...
      arguments:
        - |
          ...
          bash -c 'exportEnv "SDM__rangePredictions" "channels: [conda-forge, r]
  ...
  data>getGBIFObservations>getGBIFObservations.yml@142:
    ...
    in:
      taxa: pipeline@121
      bbox_crs: pipeline@140
      min_year: data>getGBIFObservations>getGBIFObservations.yml@142|min_year
      max_year: data>getGBIFObservations>getGBIFObservations.yml@142|max_year
      envFolder: prepareEnvironments/envFolder
      envFolderWriteable:
        default: false
      runFolder:
  ...
  data>loadFromStac.yml@144:
    ...
    in:
      ...
      t0: data>loadFromStac.yml@144|t0
      t1: data>loadFromStac.yml@144|t1
      temporal_res: data>loadFromStac.yml@144|temporal_res
      spatial_res: pipeline@128
      resampling: { default: near }
      aggregation: { default: first }
      study_area: data>loadFromStac.yml@144|study_area
      envFolder: prepareEnvironments/envFolder
      envFolderWriteable:
        default: false
      runFolder:
